...
# Tags weight
  ...
  Tags$T_SW_den<-(999.842594+0.06793952*(Tags$Temperature)-0.00909529*(Tags$Temperature)^2+0.0001001685*
    (Tags$Temperature)^3-0.000001120083*(Tags$Temperature)^4+0.000000006536332*(Tags$Temperature)^5+
      (0.824493-0.0040899* (Tags$Temperature)+0.000076438*(Tags$Temperature)^2-0.00000082467*
    (Tags$Temperature)^3+0.0000000053875*
...
BW_Genet<- ggplot(BW.data, aes (Date, BW, colour=factor(Genotype))) +
...
# 2. Water density
  ...
  BW.data$SW_den<-(999.842594+0.06793952*(BW.data$Temp)-0.00909529*(BW.data$Temp)^2+0.0001001685*
    ...
    (BW.data$Sal)+(-0.00572466+0.00010227*(BW.data$Temp)-0.0000016546*(BW.data$Temp)^2)*
    (BW.data$Sal)^1.5+0.00048314*(BW.data$Sal)^2)*0.001
...
AW_Genet2<- ggplot(BW.data, aes (Date, dAW_r, colour=Genotype)) +
  ...
  scale_y_continuous(limits = c(0, 5),
                         ...
                         expand = c(0, 0),
                         name=("Growth (mg / g d)"))+
  scale_x_date(limits = c(as.Date("2020-08-20"), as.Date("2020-10-10")),
                         breaks = "7 day",
                         expand = c(0, 0),
                         name=("Growth (mg / g d)"))+
...
BW<- ggplot(BW.data, aes (Date, Coral_BW, colour=Genotype)) +
  stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2 )+
  stat_summary(fun.y=mean, geom="line") + 
  scale_y_continuous(limits = c(-3, 10),
                         breaks = seq(-3, 10,2),  
...
D_BW<- ggplot(BW.data, aes (Date, dBW_r, colour=Genotype)) +
  stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2 )+
  stat_summary(fun.y=mean, geom="line") + 
  ...
  theme(legend.position = "bottom")
D_BW
D_BW + facet_wrap(~Nutrients)
D_BW + facet_wrap(~Disease)
