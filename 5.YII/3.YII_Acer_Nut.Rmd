...
YII_Genet<- ggplot(YII.Tall, aes (Date, YII, colour=Genotype)) +
...
YII_Genet<- ggplot(YII.Tall, aes (Date, YII, colour=factor(Nutrients), shape=Nutrients)) +
  stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2 )+
  stat_summary(fun.y=mean, geom="line") + 
...
YII_Tank<- ggplot(YII.Tall, aes (Tank, YII)) +
  ...
  scale_y_continuous(limits = c(0.4, .73),
                         ...
                         expand = c(0, 0),
                         name=("YII (Fv/Fm)"))+
  geom_jitter(aes(colour=Genotype), alpha=0.2, shape=21)+
  facet_wrap(~Time) +
  theme(legend.position = "bottom")
YII_Tank
...
YII_Frag<- ggplot(YII.Tall, aes (Date, YII, group=(Fragment),
    ...
    theme(legend.position="bottom",
        legend.title = element_blank(), 
        strip.background =element_rect(fill=NA)) 
YII_Frag+facet_wrap(Nutrients~Genotype)
...
YII_Frag1 <- ggplot(YII.Tall, aes (Date, YII, group=(Fragment),
    ...
    theme(legend.position="bottom",
        legend.title = element_blank(), 
        strip.background =element_rect(fill=NA)) 
YII_Frag1+facet_wrap(Nutrients~A_Tank)
...
YII_Nutrient<- ggplot(YII.Tall, aes (Date, YII,
                                    group=(Genotype),
                                    colour=factor(Genotype))) + 
  ...
  geom_vline(xintercept = as.Date("2020-09-14"), linetype=3)+
    ...
    theme(legend.position="bottom",
        legend.title = element_blank(), 
        strip.background =element_rect(fill=NA)) 
YII_Nutrient+facet_wrap(~Nutrients)
...
YII_Treat<- ggplot(YII.Tall, aes (Date, YII, colour=factor(Nutrients))) +   stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2)+
  stat_summary(fun.y=mean, geom="point", size =3, alpha=0.5) +
  stat_summary(fun.y=mean, geom="line") +
  ylim(0.0, 0.6) + xlim(as.Date(c("2020-09-15", "2020-11-09"))) +
   facet_wrap (~Disease) + theme_bw()
YII_Treat

YII.Tall$Days<-YII.Tall$Days-33
YII_Treat2<- ggplot(YII.Tall, aes (Days, YII, colour=factor(Nutrients))) +   stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2)+
  stat_summary(fun.y=mean, geom="point", size =3, alpha=0.5) +
  stat_summary(fun.y=mean, geom="line") +
  ylim(0.0, 0.6) + xlim(c(1, 60)) +
   facet_wrap (~Disease) + theme_bw() + geom_vline(xintercept = 46, linetype="dashed", 
                color = "gray")
YII_Treat2

   #ggsave("YII.svg", YII_Treat2, width=6, height=4,dpi = 300)
