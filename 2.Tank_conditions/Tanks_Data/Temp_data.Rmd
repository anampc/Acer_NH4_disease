---
title: "Import and clean acer Temps nutrients and disease"
author: "Ana Palacio"
date: "`r Sys.Date()`"
output:
  html_document:
...
# Data types
    Temperature.tall$Temperature<-as.numeric(Temperature.tall$Temperature)
    Temperature.tall$Tank<-as.factor(Temperature.tall$Tank)
    ...
    summary(Temperature.tall)
    Temperature.tall<-Temperature.tall %>% drop_na(Temperature)
...
```{r}
TankTemperature<- ggplot(Temperature.tall, aes (Time, Temperature, group=Tank)) +
...
write.csv(Experiment, "Temp_experiment.csv", row.names = F)
