...
# Only treatment model
    ...
    fit1 <- survfit(surv_object ~ Treatment,
                    data = Survival.data)
    summary(fit1)
    coxfit <- coxph(surv_object ~ Treatment, data = Survival.data)
    summary(coxfit)
...
# Only genotype model
  ...
  # Plot the survival model
    ...
    fit0 <- survfit(surv_object_1 ~ Treatment, conf.type = "log-log", data = Survival.data_1)
    summary(fit0)
  # Plot the survival model
    Treatment_only<-ggsurvplot(fit0, data = Survival.data_1, pval = TRUE,
              risk.table=F, 
...
# Kaplan-Meier estimator. The "log-log" confidence interval is- preferred.
    ...
    TreatmentP<-ggsurvplot_facet(fit2, data = Survival.data_1, 
      ...
      annotate("point",x=c(58), y=c(0), 
           shape=4, size=2)
    TreatmentP
    #ggsave("Outputs/Samples2.svg", print(TreatmentP), width=4.5, height=4.5,dpi = 300)
```
