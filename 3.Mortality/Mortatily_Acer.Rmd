...
# Data
    ...
    Survival.data$Gen_Treat<-paste(Survival.data$Genotype,
                                   Survival.data$Gen_Treat, sep = "-" )
    Survival.data$Genotype<-as.factor(Survival.data$Genotype)
```

# 1. Experimental data (excluding cooked tank)
...
```{r}
    fit1 <- survfit(surv_object_1 ~ Treatment, conf.type = "log-log", data = Survival.data_1)
    summary(fit1)
  # Plot the survival model
    ...
    Treatment_only
    #ggsave("Outputs/Tratment.svg", Treatment_only, width=6, height=6,dpi = 300)
```

## Model 2 (Genotype + Treatemnt)
...
# Kaplan-Meier estimator. The "log-log" confidence interval is- preferred.
    ...
    TreatmentP<-ggsurvplot_facet(fit2, data = Survival.data_1, 
      ...
      annotate("point",x=c(58), y=c(0), 
           shape=4, size=2)
    TreatmentP
  #ggsave("Outputs/Samples2.svg", print(GenotypeP), width=5.5, height=8,dpi = 300)
```
...
## 2.1 Ambient-Pathogen

```{r}
# Data
    Survival.data2<-Survival.data_1[(Survival.data_1$Treatment=="Ambient-Pathogen"),]
...
# Data
  ...
  ## Add survival object (Fit survival data using the Kaplan-Meier method)
  surv_object3 <- Surv(time = Survival.data3$Fu.time_texp, event = Survival.data3$Fu.stat_exp)
  surv_object3 
    ```
...
```{r}
# Data
    Survival.data4<-Survival.data_1[(Survival.data_1$Treatment=="NH4-Pathogen"),]
  ...
  ## Add survival object (Fit survival data using the Kaplan-Meier method)
  surv_object4 <- Surv(time = Survival.data4$Fu.time_texp, event = Survival.data4$Fu.stat_exp)
  surv_object4 
    ```
...
# Data
    ...
    Survival.data5$Treatment<-factor(Survival.data5$Treatment, 
                                     levels = c("NH4-Placebo", "Ambient-Pathogen", "NH4-Pathogen"))
  ...
  ## Add survival object (Fit survival data using the Kaplan-Meier method)
  surv_object5 <- Surv(time = Survival.data5$Fu.time_texp, event = Survival.data5$Fu.stat_exp)
  surv_object5 
